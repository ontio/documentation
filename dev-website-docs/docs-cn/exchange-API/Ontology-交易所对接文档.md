...
>强制要求：交易所必须使用白名单或防火墙来屏蔽外部服务器请求，否则可能会产生重大安全隐患。
...
3. 通过 CLI  ```./ontology info status```根据 ```Transaction Hash``` 取得 ```block``` 中的所有 ```Transaction``` 信息
...
用户提取 ```ONG``` 的流程和提取 ```ONT``` 的流程一致，只需指定 ```asset``` 参数为 ```ong``` 即可：
