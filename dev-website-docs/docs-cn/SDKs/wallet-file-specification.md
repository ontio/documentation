...
钱包文件数据以 `json` 格式存储，文件后缀名并无要求，钱包文件数据结构如下:
...
Control 是数字身份的控制人，里面包含控制人的加密后算法名，私钥，公钥，地址，salt 等。
...
```salt``` 16 字节的盐，以 base64 格式存储.


#### 1.2.2 Parameter 数据结构
